...
start_server() {
    ...
    # inside Docker. The container process runs with the host user's UID
    # and the docker group GID so it can write to bind-mounted volumes.
    local host_uid=$(id -u)
...
setup_host_directory() {
    ...
    # Use the host user's UID:GID so the Docker container (which runs
    # with --user matching the host user) can read and write all files.
    chown -R "$(id -u):$(id -g)" "$BINARIES_DIR" 2>/dev/null || true
    chown "$(id -u):$(id -g)" "$BASE_DIR/umu-data" 2>/dev/null || true
    chmod -R u+rwX,g+rwX,o-rwx "$BINARIES_DIR" 2>/dev/null || true
    chmod u+rwX,g+rwX,o-rwx "$BASE_DIR/umu-data" 2>/dev/null || true
    # Wipe ACLs — they can interfere with Docker bind mounts
...
update_ark_binaries() {
    ...
    # No --user flag: steamcmd needs to write to the volume, which is root-owned.
    # The update container runs as root (default); only the server container needs --user for umu-launcher.
    docker run --rm \
      -v "$BINARIES_DIR:/ark/binaries" \
      --env UMASK=0007 \
      "$IMAGE_NAME" update
